...
class LegendreKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_normalized = F.tanh(x * self.tanh_scale + self.tanh_bias) # this is better
...
class BetterLayer(nn.Module):
    def __init__(self, in_features, out_features, polynomial_order=3, base_activation=nn.SiLU):
        ...
        self.tanh_scale = nn.Parameter(torch.randn(1, in_features))
        self.tanh_bias = nn.Parameter(torch.randn(1, in_features))

        nn.init.kaiming_uniform_(self.base_weight, nonlinearity='linear')
        nn.init.kaiming_uniform_(self.poly_weight, nonlinearity='linear')
        nn.init.kaiming_uniform_(self.tanh_scale, nonlinearity='linear')
        nn.init.kaiming_uniform_(self.tanh_bias, nonlinearity='linear')
